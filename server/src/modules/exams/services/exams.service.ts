...
export class ExamsService {
  ...
  async getOrGenerateTestForLesson(
    generateTestDto: GenerateTestDto,
    userId: string,
  ) {
  ...
  async submitTestResult(
    submitTestResultDto: SubmitTestResultDto,
    userId: string,
  ) {
    ...
    for (const examResult of exam.results) {
      const submitted = submittedAnswersByQuestionId.get(
        examResult.question.id,
      );
      ...
      if (
        ...
        const normalizedUserAnswer = examResult.user_answer
          .trim()
          .toLowerCase();
        const normalizedCorrectAnswer = examResult.question.correct_answer
          .trim()
          .toLowerCase();
        examResult.is_correct =
  ...
  async deleteExamsByLesson(lessonId: string, userId: string) {
    ...
    if (lesson.user.id !== userId) {
      throw new ForbiddenException(
        'You can only reset progress for your own lesson',
      );
    }
