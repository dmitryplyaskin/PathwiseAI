...
import type { CourseListItem, LessonDetail } from '@shared/api/courses/types';
import { coursesApi } from '@shared/api/courses/api';

interface CourseCardWithLessonsProps {
...
const getLessonStatusIcon = (status: string) => {
  switch (status) {
    ...
    case 'learning':
      return <PlayCircle color="primary" fontSize="small" />;
    case 'not_started':
...
export const CourseCardWithLessons: FC<CourseCardWithLessonsProps> = ({
  course,
  handleCourseClick,
}) => {
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      ...
      try {
        ...
        setLessons(courseLessons.slice(0, 3));
      } catch (error) {
        setLessonsError('Ошибка загрузки уроков');
        ...
        <Stack spacing={2} height="100%">
          ...
          {course.progress > 0 && (
            <Box>
              ...
              <Typography
                ...
                fontWeight={500}
              >
                Прогресс: {course.completedLessons || 0}/
                {course.totalLessons || 0} уроков
              </Typography>
            </Box>
          )}
          ...
          <Box>
            ...
            {lessonsLoading ? (
              ...
              <List dense sx={{ py: 0 }}>
                {lessons.map((lesson) => (
                  <ListItem key={lesson.id} sx={{ px: 0, py: 0.5 }}>
          ...
          <Stack spacing={1}>
            ...
            <Box display="flex" alignItems="center" gap={2}>
              <Box display="flex" alignItems="center" gap={0.5}>
                ...
                <Typography variant="caption" color="text.secondary">
                  {course.studentsCount?.toLocaleString() || 0} студентов
                </Typography>
              </Box>
              <Box display="flex" alignItems="center" gap={0.5}>
                <Rating
