...
- `GET /lessons/for-review` - получение уроков для повторения текущего пользователя
...
- Блок **"Повторить"** использует endpoint `GET /lessons/for-review` и показывает top-N уроков.
...
1. **API:** Используйте endpoint `/lessons/for-review` для получения уроков
