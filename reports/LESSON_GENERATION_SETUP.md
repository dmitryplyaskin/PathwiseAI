...
3. **AI промпт для генерации урока**
   ...
   - Генерирует структурированный контент в **Markdown** (контент урока — markdown-строка внутри JSON, не HTML)
   - Включает введение, основной материал, примеры и резюме
   - Защита от prompt injection: `topic/details` считаются недоверенными данными (инструкции внутри них игнорируются)
   - Анти-галлюцинации: запрещены “уверенные” выдуманные факты (точные цифры/даты/цитаты/ссылки); при неопределённости — осторожные формулировки
